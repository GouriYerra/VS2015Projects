...
// ~markov() == Releases any models created in constructor.
markov::~markov() {
...
// run() == Launches a user-interactive Markov application session.
void markov::run() {
	cout << "-+== Welcome to the Markov random text generator! ==+-" << endl;
	...
	while (true) {
		...
		if (_random_seed != -1) {
			cout << "The current random seed is " << _random_seed << "." << endl;
		}
...
// run_one(...) == Sets up a model as specified, generates text, then exits.
...
// set_text() == Ask the user for a new text file to load as an example text.
...
// set_order() == Ask the user for a new model order.
...
// set_model() == Ask the user for a model type to use.
...
// initialize_model() == Initialize (or re-initialize) the model object with the current text and order.
...
// _mark_start() == Record the starting processor time.
void markov::_mark_start() {
	_start_time = clock();
}

// _mark_stop() == Record the ending processor time.
void markov::_mark_stop() {
	_stop_time = clock();
}

// _report_time() == Output a report of elapsed time.
